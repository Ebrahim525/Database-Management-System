...
#define PRINT_TABLE "\\s*PRINT\\s+TABLE\\s+([A-Za-z0-9_-]+)\\s*;?"
#define LIST_ALL "\\s*ls\\s*;?"

/* Format Disk */
#define FORMAT_DISK "\\s*fdisk\\s*;?"
...
class RegexHandler {
 ...
 private:
  ...
  const std::vector<std::pair<std::regex, handlerFunction>> handlers = {
      ...
      {REGEX(PRINT_TABLE), &RegexHandler::printTable},
      {REGEX(LIST_ALL), &RegexHandler::ls},
      {REGEX(FORMAT_DISK), &RegexHandler::fdisk},
  };
